...
export default function AgendamentoItem(props: AgendamentoItemProps) {
  const cor =
    new Date(props.agendamento.data).getTime() > Date.now()
      ? "#007aff"
      : "#AAAAAA";
  ...
  function somarTotalServicos() {
    return props.agendamento.servicos.reduce(
    ...
    <View style={{ ...styles.cartao, borderColor: cor }}>
      ...
      <Text style={{ ...styles.data, color: cor }}>
        {props.agendamento.data &&
          formatarData(new Date(props.agendamento.data))}
        {props.agendamento.data &&
          formatarHorario(new Date(props.agendamento.data))}
      </Text>
